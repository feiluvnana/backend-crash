...
Reference auth/user implementation lives in `examples/user_auth_reference/`.
...
| `migration` | `migration/` | SeaORM migration runner |
| `generator` | `generator/` | Feature scaffolding CLI |

```
src/
├── main.rs              # Config → DB → Router → serve with graceful shutdown
├── lib.rs               # Re-exports: db, features, infra, middleware, routes
├── db/setup.rs          # connect_db(url, max_conn, min_conn)
├── features/            # Domain modules (handler + service + dto pattern)
│   └── health/handler.rs
├── infra/
│   ├── config.rs        # Config from env vars via dotenvy
│   ├── error.rs         # AppError enum → IntoResponse + From<DbErr>
│   ├── extractor.rs     # ValidatedJson<T> custom Axum extractor
...
Place in `src/db/models/`. Register in `src/db/models/mod.rs` → `src/db/mod.rs`.
...
6. **Integration tests** use `tests/common/spawn_app()` which starts a real server on a random port.
7. **Migrations** go in `migration/` crate. Register in `migration/src/lib.rs`.
8. **`generator/`** is a separate workspace member — does not affect main app compile times.
9. **`examples/user_auth_reference/`** is the canonical reference for auth, JWT, user CRUD, and custom extractors (like `CurrentUser`). Copy and adapt from there.
10. **No business-specific deps in root `Cargo.toml`** unless used by core infra.
